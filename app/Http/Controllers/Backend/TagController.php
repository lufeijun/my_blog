...
class TagController extends Controller
{
    ...
    public function store(CreateRequest $request)
    {
        ...
        if($this->tag->create($data)){
            return redirect('/lufeijun/tag')->with('success','添加标签成功');
        }

        return redirect(route('lufeijun.tag.create'))->withErrors('标签添加失败');
    }
    ...
    public function update(UpdateRequest $request,$id)
    {
        ...
        if($this->tag->update($data,$id)){
            return redirect('/lufeijun/tag')->with('success','标签修改成功');
        }
